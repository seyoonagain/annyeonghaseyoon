...
const Portfolio = () => {
  ...
  return (
    <div className="flex flex-col-reverse sm:flex-row-reverse justify-center w-full sm:max-w-2xl h-80 animate-bgColorShift">
      {projects.map(({ description, slug, team }, index) => (
        <Link
          href={`/portfolio/projects/${slug}`}
          key={slug}
          className="flex flex-row sm:flex-col items-start gap-2 shrink-0 relative w-full sm:w-1/3 h-1/3 sm:h-full px-4 py-1 border border-x-0 sm:border-x sm:border-y-0 not-first:border-b-0 sm:not-first:border-r-0 bg-white hover:bg-transparent font-manrope cursor-pointer"
